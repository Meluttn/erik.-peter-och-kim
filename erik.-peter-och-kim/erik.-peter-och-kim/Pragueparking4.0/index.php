...
switch ($action) 
{
    case 'park': // Om park kör filtreringsmetod
        ...
        $carUnit = new Vehicles($regNR, $size, $_POST['vType'], $price); //skapar nytt fordon med användarinput
        $emtyspot = readFromFile();
        
        function getParking($carUnit,$emtyspot) // hämtar fordons parkeringsplats
        {
            for ($i=0;$i<count($emtyspot);$i++)
            {
                if($emtyspot[$i][5]==0)
                {
                    return $i;
        ...
        if(checkVehiclesExists($regNR,$emtyspot)) 
        {
            echo 'Fail, already parked this one!';
        }

        // var_dump($emtyspot);
        // writeToFile($carUnit);

        elseif(!empty($emtyspot) && count($emtyspot) <= 10 )
        {
          appendToFile($carUnit);
        
        }
        
        else 
        echo 'Fail, youve tried to add too many vehicles! ';
        break;
        
    }
    function checkVehiclesExists($regNR,$emtyspot)
    {
        for($i=0;$i<count($emtyspot);$i++)
        {
        if(in_array($regNR,$emtyspot[$i]))
        {
...
function readFromFile()
{
    ...
    $fp=fopen('csv/parkedvehicles.csv', 'r');
    while (!feof($fp) ) {
        $lines[] = fgetcsv($fp, 1000, ',');
    }
    fclose($fp);
    return $lines;
    ...
    $fp=fopen('csv/parkedvehicles.csv', 'w');
    ...
    function appendToFile($carUnit)
    {
    $carArr=(array)$carUnit;
    $fp=fopen('csv/parkedvehicles.csv', 'a');
